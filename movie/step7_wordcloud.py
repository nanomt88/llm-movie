...
setup_matplotlib()
STEP_OUT = STEP_DIRS[7]
os.makedirs(STEP_OUT, exist_ok=True)

# ── Stopwords ─────────────────────────────────────────────────────────
# Standard English stopwords + domain-specific noise
STOPWORDS = set({
...
# Additional domain-specific noise
...
def tokenize(text: str) -> list[str]:
    ...
    return [t.strip("'") for t in tokens
            if len(t.strip("'")) > 2
            and t.strip("'") not in ALL_STOPWORDS
...
def plot_top_words_bar(
    ...
    """Grouped bar chart comparing top word frequencies across groups.
       分组柱状图：比较不同分组的高频词频次。"""
    # Collect top words across all groups
    all_words = set()
...
def plot_holiday_elevated_words(
    ...
    """Plot words where holiday avg > non-holiday avg * threshold.
       绘制节假日日均频次显著高于非节假日的单词（可配置比值阈值）。"""
    candidates = []
    for w, h_val in h_avg.items():
        nh_val = nh_avg.get(w, 0)
        if h_val > nh_val * threshold:
            candidates.append((w, h_val, nh_val))
    candidates.sort(key=lambda x: x[1], reverse=True)  # sort by holiday avg desc
...
def _save_word_csv(
    ...
    # Sort by total across all groups
    sorted_words = sorted(
        all_words,
...
def dim_w3_holiday_workday_weekend_words(seekers: list[dict]):
    ...
    for p in ['holiday', 'workday', 'weekend']:
        ...
        freq_dict[p.capitalize()] = {
            w: c / max(len(p_dates), 1) for w, c in pf.items()
        }
...
def dim_w4_per_holiday_words(seekers: list[dict], top_n: int = 20):
    ...
    # Compute per-holiday avg daily freq and ratio vs baseline
    holiday_avg = {}
    holiday_ratio = {}
    all_words = set()
    ...
    # ── CSV: word × holiday matrix with ratio columns ──
    csv_path = os.path.join(STEP_OUT, 'w4_per_holiday_words.csv')
    # Composite score: for each word, max(ha * min(ratio, 20)) across holidays
    word_score = {}
    for w in all_words:
        max_score = 0
        for hn in holiday_names:
    ...
    # ── Per-holiday bar charts (one subplot per holiday) ──
    ...
    for idx, hn in enumerate(holiday_names):
        ...
        # Score each word: h_avg * min(ratio, 20)
        scored = []
        for w, ha in h_avg_dict.items():
            ratio = min(holiday_ratio[hn].get(w, 1), 20)
            scored.append((w, ha, ratio, ha * ratio))
        scored.sort(key=lambda x: x[3], reverse=True)  # sort by composite score
        top = scored[:top_n]
    ...
    # ── Console log: per-holiday top words ──
...
def dim_w5_per_holiday_words_heatmap(seekers: list[dict]):
    ...
    EPSILON = 1e-6  # prevent log(0) for words with zero baseline
    ...
    # Compute per-holiday avg daily freq and log2 ratio
    holiday_avg = {}
    all_words = set()
    for hn in holiday_names:
        h_dates = set(r['date'] for r in holiday_groups[hn])
    ...
    # Select top 60 words: by max composite score across all holidays
    word_score = {}
    for w in all_words:
        max_score = 0
        for hn in holiday_names:
    ...
    # Build log2 ratio matrix, clipped to [-3, 3]
    ...
    # Plot heatmap — symmetric fixed range makes colors comparable across runs
    fig_w = max(12, n * 1.2)
...
def main(data: dict = None, ratio_threshold: float = 1.5):
    ...
    dim_w1_overall_wordcloud(seekers)
    log("")
    dim_w2_holiday_vs_nonholiday_words(seekers, ratio_threshold=ratio_threshold)
    log("")
    dim_w3_holiday_workday_weekend_words(seekers)
    log("")
    dim_w4_per_holiday_words(seekers)
    log("")
    dim_w5_per_holiday_words_heatmap(seekers)
