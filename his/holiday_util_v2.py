# -*- coding: utf-8 -*-
"""
节假日工具模块 v2：加载 holiday.csv 并提供日期是否为节假日的判断功能。
可与 extrace.py 中的 MovieRecord / pd.DataFrame 配合使用。

新增功能（main）：
  - 从 my-train-data.csv 中提取节假日记录，写入 my-tran-holidy-data.csv
"""
...
if __name__ == '__main__':
    ...
    # csv_candidates = [f for f in os.listdir(data_dir) if f.endswith('data.csv') and f != 'holiday.csv']
    # if csv_candidates:
    ...
    # ══════════════════════════════════════════════════════════════════
    #  示例 5：从 my-train-data.csv 中提取节假日记录（含 user+system 配对）
    # ══════════════════════════════════════════════════════════════════
    #
    # 逻辑说明：
    #   1. 先找出所有在节假日的用户提问记录（is_seeker=True, holiday=True）
    #   2. 对每条用户提问，在同一 conv_id 内按 turn_order 找到紧随其后的
    #      system 回答（is_seeker=False），一并提取
    #   3. user 和 system 在输出中保持前后相邻，user 提问顺序不变
    #
    train_path = os.path.join(data_dir, 'my-train-data.csv')
    holiday_out_path = os.path.join(data_dir, 'my-tran-holidy-data.csv')

    print(f"\n--- 从 my-train-data.csv 提取节假日记录（含 user+system 配对）---")
    if os.path.exists(train_path):
        df_train = pd.read_csv(train_path)
        df_tagged = cal.add_holiday_flags(df_train, col='utc_time')

        def _collect_holiday_pairs(grp: pd.DataFrame) -> pd.DataFrame:
            """
            处理单个 conv_id，提取节假日 user + 对应的 system 回答。

            规则：
              - 找到 is_seeker=True 且 holiday=True 的用户提问
              - 提取该提问记录
              - 提取紧随其后的所有 system 回答（直到遇到下一条用户提问或 conv_id 结束）
            """
            grp = grp.sort_values('turn_order')
            holiday_users = grp[
                (grp['is_seeker'] == True) & (grp['holiday'] == True)
            ]
            if holiday_users.empty:
                return grp.iloc[0:0]  # 返回空 DataFrame

            selected_indices = []
            for idx in holiday_users.index:
                pos = grp.index.get_loc(idx)
                # 添加 user 记录本身
                selected_indices.append(grp.index[pos])
                # 添加后续的 system 记录
                j = pos + 1
                while j < len(grp) and grp.iloc[j]['is_seeker'] == False:
                    selected_indices.append(grp.index[j])
                    j += 1

            return grp.loc[selected_indices]

        # 按 conv_id 分组提取
        df_holiday_pairs = (
            df_tagged
            .groupby('conv_id', group_keys=False)
            .apply(_collect_holiday_pairs)
        )

        # 恢复原始数据顺序（按原始 CSV 中的排列顺序）
        df_holiday_pairs = df_holiday_pairs.reindex(
            sorted(df_holiday_pairs.index)
        )

        # 删除辅助标记列，保持原文件格式不变
        df_holiday_pairs.drop(
            columns=['holiday', 'holiday_name', 'holiday_type'],
            inplace=True,
        )

        # 写入 CSV（不写索引）
        df_holiday_pairs.to_csv(holiday_out_path, index=False)

        # 统计
        n_user = df_holiday_pairs['is_seeker'].sum()
        n_system = len(df_holiday_pairs) - n_user
        print(f"  my-train-data.csv 总记录: {len(df_train)}")
        print(f"  节假日用户提问: {n_user} 条")
        print(f"  对应 system 回答: {n_system} 条")
        print(f"  提取总记录: {len(df_holiday_pairs)} 条")
        print(f"  已写入: {holiday_out_path}")
    else:
        print(f"  未找到 {train_path}，跳过")
