...
def generate_heatmaps(records: list[dict]):
    ...
    for age in age_order:
        ...
        _plot_heatmap(
            ...
            columns='holiday_name''',  # 横轴：节假日名称
            title=f'{age} — 假日场景下用户情感分析热图',
            filename=f'v3_heatmap1_holiday_sentiment_{safe_age}.png',
        )

        # ── 热图 2: 情感 × 影片类型 ──
        # 目的：观察用户在不同情感下更喜欢看什么类型的电影
        _plot_heatmap(
            data=sub,
            index='genre',       # 纵轴：情感分类
            columns='sentiment',       # 横轴：电影类型
        ...
        _plot_heatmap(
            data=sub,
            index='genre',        # 纵轴：节假日名称
            columns='holiday_name',   # 横轴：电影类型
